...
for i in ${ID}
do
/path/to/canu -s pacbio.spec -p ${i} -d ${i} genomeSize=600m -pacbio-raw RawPacBio.fasta
...
bwa index ${i}_SuperContig.fasta
bwa mem -t 10 ${i}_SuperContig.fasta R1.fastq.gz R2.fastq.gz | samtools sort -@ 10 -O bam -o align.bam
...
java -jar -Xmx1000G pilon-1.22.jar --genome ${i}_SuperContig.fasta --frags align_filter.bam --fix snps,indels --output pilon_polished --vcf --targets Name.txt &> pilon.log
